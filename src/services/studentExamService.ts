...
export class StudentExamService {
    ...
    private assertInWindow(exam: PublicExamRow): void {
        ...
        if (now < new Date(exam.startDate) || now > new Date(exam.endDate)) {
            throw new DomainError("Examen hors de sa fenêtre de disponibilité");
        }
    }
    ...
    private parseSubmission(body: unknown): SubmittedAnswer[] {
        ...
        if (!Array.isArray(answers)) {
            throw new DomainError("Données invalides : answers est requis");
        }
        ...
        for (const raw of answers) {
            ...
            if (!Number.isInteger(questionId) || !Number.isInteger(choiceId)) {
                throw new DomainError("Données invalides : chaque réponse requiert questionId et choiceId");
            }
            if (submitted.some((answer) => answer.questionId === questionId)) {
                throw new DomainError("Données invalides : réponses dupliquées pour une même question");
            }
    ...
    async getDetail(examId: number, studentId: string): Promise<PublicExamDetail> {
        ...
        if (alreadyAttempted) {
            throw new DomainError("Vous avez déjà passé cet examen");
        }
    ...
    async submit(examId: number, studentId: string, body: unknown): Promise<ExamResult> {
        ...
        try {
            ...
            if (alreadyAttempted) {
                throw new DomainError("L'étudiant a déjà passé cet examen");
            }
            ...
            for (const answer of submitted) {
                ...
                if (!choice || choice.questionId !== answer.questionId) {
                    throw new DomainError("Données invalides : choix inconnu pour une question de cet examen");
                }
                if (choice.isCorrect) {

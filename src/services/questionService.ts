...
export class QuestionService {
    ...
    private parsePayload(body: unknown): QuestionInput {
        ...
        if (typeof text !== "string" || text.trim().length === 0) {
            throw new DomainError("Données invalides : text est requis");
        }
        const parsedScore = Number(score);
        if (!Number.isInteger(parsedScore) || parsedScore < 1) {
            throw new DomainError("Données invalides : score doit être un entier >= 1");
        }
        if (!Array.isArray(choices) || choices.length < 2 || choices.length > 6) {
            throw new DomainError("Données invalides : entre 2 et 6 choix requis");
        } 
        ...
        const parsedChoices: ChoiceInput[] = choices.map((choice) => {
            ...
            if (typeof label !== "string" || label.trim().length === 0) {
                throw new DomainError("Données invalides : chaque choix requiert un label");
            }
            if (typeof isCorrect !== "boolean") {
                throw new DomainError("Données invalides : isCorrect est requis pour chaque choix");
            }
        ...
        if (correctCount !== 1) {
            throw new DomainError("Données invalides : une seule réponse correcte requise");
        }
    ...
    async update(id: number, body: unknown) {
        ...
        try {
            ...
            if (hasAttempts) {
                throw new DomainError("Modification refusé : l'examen a déjà au moins une tentative");
            }
      ...
      async remove(id: number): Promise<void> {
        ...
        if (hasAttempts) {
            throw new DomainError("Suppression refusé : l'examen a déjà une tentative");
        }
